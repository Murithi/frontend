...
import { Header, Table,  Message, Icon } from 'semantic-ui-react';
...
class MaterialRequisitionsApproved extends Component {
    ...
    render() { 
        ...
        if (this.props.requisitionFeed && this.props.requisitionFeed.loading) {
          ...
          if (this.props.requisitionFeed.approvedMaterialRequisitionsFeed === undefined ||this.props.requisitionFeed.approvedMaterialRequisitionsFeed.length === 0) {
        ...
        return ( 
            ...
            <Header as="h4" color="green" textAlign="center">
                ...
                <Table>
                    ...
                    <Table.Body>
                        {   
                            this.props.requisitionFeed.approvedMaterialRequisitionsFeed.map(request => {
                               console.log(request)
                                if (request.paymentMode === 'CHEQUE' && request.otherPaymentDetails === null) {                                 
                                    return (
                                    <Table.Row>
                                            ...
                                            if (this.props.userDetails.me.role === 'ACCOUNTANT') {
                                                return (
                                                    <Link to={`/materialrequisitions/issuecheque/${request.id}`}>
                                                        Issue Cheque
                                ...
                                else if (request.paymentMode === 'CASH' && request.cashPaymentsDetails === null){ 
                                    return (
                                    <Table.Row>
                                            <Table.Cell>{request.materialType.materialName}</Table.Cell>
                                            <Table.Cell>{request.proposedSupplier.supplierName}</Table.Cell>
                                            ...
                                            if (this.props.userDetails.me.role === 'ACCOUNTANT') {
                                                return (
                                                    <Link to={`/materialrequisitions/issuecash/${request.id}`}>
                                                        Issue Cash
                                                    <Icon name="angle double right" />
                                                    </Link>
                                                )
